...
KEYPAD_ERR_STATUS KEYPAD_u8Init(void){
	...
	status = DIO_u8SetPinDirection(COL0_PORT, COL0_PIN, OUTPUT_PIN);
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinDirection(COL1_PORT, COL1_PIN, OUTPUT_PIN);	
	}
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinDirection(COL2_PORT, COL2_PIN, OUTPUT_PIN);
	}
	
	//Set Cols high
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinData(COL0_PORT, COL0_PIN, HIGH);
	}
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinData(COL1_PORT, COL1_PIN, HIGH);
	}
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinData(COL2_PORT, COL2_PIN, HIGH);
	}
	
	//Set rows direction input with pullup
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinDirection(ROW0_PORT, ROW0_PIN, INPUT_PIN_PULLUP);
	}
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinDirection(ROW1_PORT, ROW1_PIN, INPUT_PIN_PULLUP);
	}
	if(status == KEYPAD_OK){
		status = DIO_u8SetPinDirection(ROW2_PORT, ROW2_PIN, INPUT_PIN_PULLUP);
	}
	...
	if(key == NULL){
		...
		switch(key_state){
			...
			if(status == KEYPAD_OK && *key != NO_KEY_PRESSED){
				if(TIMER2_u8InterruptDelay(DEBOUNCING_DELAY, KEYPAD_CallBack) == DELAY_OK){
	...
	if(key == NULL){
		...
		*key = NO_KEY_PRESSED;
		for(uint8_t col = 0; col < KEYPAD_COLS; col++){
			switch(col){
				case  0:
					status = DIO_u8SetPinData(COL0_PORT, COL0_PIN, LOW);
					break;
				case 1:
					status = DIO_u8SetPinData(COL1_PORT, COL1_PIN, LOW);
					break;
				case 2:
					status = DIO_u8SetPinData(COL2_PORT, COL2_PIN, LOW);
					break;
				default:
					break;
			}
