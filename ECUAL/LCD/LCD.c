...
LCD_ERR_STATUS LCD_u8Init(void) {
	...
	DIO_u8SetPinDirection(RS_PORT, RS_PIN, OUTPUT_PIN);
	DIO_u8SetPinDirection(RW_PORT, RW_PIN, OUTPUT_PIN);
	DIO_u8SetPinDirection(E_PORT, E_PIN, OUTPUT_PIN);
	DIO_u8SetPinDirection(D4_PORT, D4_PIN, OUTPUT_PIN);
	DIO_u8SetPinDirection(D4_PORT, D5_PIN, OUTPUT_PIN);
	DIO_u8SetPinDirection(D4_PORT, D6_PIN, OUTPUT_PIN);
	DIO_u8SetPinDirection(D4_PORT, D7_PIN, OUTPUT_PIN);
	TIMER0_u8Init();
	TIMER0_u8InterruptDelay(LCD_PERIOD, LCD_callBack);
...
static LCD_ERR_STATUS LCD_u8SendCommandPrivate(uint8_t command){
	...
	switch(lcd_state){
		...
		DIO_u8SetPinData(RS_PORT,RS_PIN,LOW);
		DIO_u8SetPinData(RW_PORT,RW_PIN,LOW);
		DIO_u8SetPinData(E_PORT,E_PIN,HIGH);
...
LCD_ERR_STATUS LCD_u8SendDataPrivate(uint8_t data) {
	...
	switch(lcd_state){
		...
		bit = GET_BIT(data, 7);
		DIO_u8SetPinData(D7_PORT,D7_PIN,bit);
		DIO_u8SetPinData(RS_PORT,RS_PIN,HIGH);
		DIO_u8SetPinData(RW_PORT,RW_PIN,LOW);
		DIO_u8SetPinData(E_PORT,E_PIN,HIGH);
		lcd_state = SECOND_NIBBLE;
		status = LCD_BUSY;
